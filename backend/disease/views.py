...
import difflib
from disease.models import Prediction, Treatment
...
disease = ['Fungal infection', 'Allergy', 'Gastroesophageal reflux disease (GERD)', 'Chronic cholestasis',
           ...
           'Hepatitis B liver infection', 'Hepatitis C virus infection', 'Hepatitis D swelling liver',
...
l1 = ['Body back pain', 'constipation', 'abdominal pain', 'diarrhea', 'mild_fever', 'yellowish urine',
      'yellowish eyes', 'damage of liver', 'too much fluids in the blood', 'swelling of stomach',
      'swelled lymph nodes', 'malaise', 'Impaired vision', 'phlegm', 'inconvenience of the throat',
      'Redness of eyes', 'Nasal congestion', 'discharge mucus from the nose', 'Body stiffness', 'chest pain',
      'weakness in limbs',
      ...
      'swelling of arms, feet, ankles, and legs', 'excessive hunger', 'Problems with unmarried relationships',
      ...
      'depression', 'irritability', 'muscle pain', 'inability to think clearly or concentrate', 'red spots on body',
      'lower stomach pain',
      'abnormal menstruation', 'Discolored skin', 'Tears in the eyes', 'Excessive hunger',
      'production of abnormal volumes of urine', 'Hereditary disorders', 'white mucus come from the nose',
      ...
      'high dosage of alcohol consumption', 'Excess fluid resulting in body weight gain', 'blood with mucus',
      'swollen and enlarged veins that occur on the legs and feet',
      'feel like heart stopped beats', 'pain when walking', 'pus-filled pimples', 'blackheads', 'scurring',
      'damage to the upper layer of your skin',
      'the skin turns blue or blue-grey', 'pitting in Nail', 'an inflammation of the skin around the nail', 'blister',
      'Red skin under and around nose',
...
def predictDisease(request):
    ...
    prediction.name = translator.translate(request.POST.dict()['symptom1']).text
    prediction.symptom1 = translator.translate(request.POST.dict()['symptom1']).text
    prediction.symptom1 = translator.translate(request.POST.dict()['symptom2']).text
    prediction.symptom1 = translator.translate(request.POST.dict()['symptom3']).text
    ...
    treatment = Treatment.objects.get(B=predicted)

    data = translator.translate(predicted, dest='si')

    return render(request, 'result.html', {'data': data.text, 'treatment': treatment})
